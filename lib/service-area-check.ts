...
export async function checkServiceAddress(input: { address: string; city: string; state: string; zip: string }): Promise<{ error: string; status: number } | null> {
  ...
  if (!address.trim() || !city.trim() || !/^\d{5}(?:-\d{4})?$/.test(zip.trim())) {
    return { error: "Enter the full service street address, city and ZIP to check our 50-mile service area.", status: 400 };
  }
  if (!/^(GA|Georgia)$/i.test(state.trim())) {
    return { error: "This address is outside DCA's 50-mile Atlanta service area. No order or deposit was created.", status: 422 };
  }
  if (!serverKey()) {
  ...
  if (!withinServiceRadius(location, serviceCenter())) {
    return { error: `This service address is ${Math.round(milesBetween(location, serviceCenter()))} miles from our Atlanta service center, outside our 50-mile radius. No order or deposit was created.`, status: 422 };
  }
  return null;
}
